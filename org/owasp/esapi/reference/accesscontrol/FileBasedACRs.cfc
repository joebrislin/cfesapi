...
<cfcomponent extends="cfesapi.org.owasp.esapi.lang.Object" output="false">
	...
	<cfscript>
		...
		instance.deny = newComponent("cfesapi.org.owasp.esapi.reference.accesscontrol.Rule").init();
	...
	<cffunction access="public" returntype="boolean" name="isAuthorizedForURL" output="false"
	            hint="Check if URL is authorized.">
	...
	<cffunction access="private" returntype="boolean" name="matchRule" output="false"
	...
	<cffunction access="private" returntype="boolean" name="matchClassRule" output="false"
		...
		<cfscript>
			...
			return structKeyExists(local, "rule");
		</cfscript>

	</cffunction>

	<cffunction access="private" returntype="Rule" name="searchForRule" output="false"
		...
		<cfscript>
			...
			if(local.part.indexOf("..") != -1) {
				throwError(newComponent("cfesapi.org.owasp.esapi.errors.IntrusionException").init(instance.ESAPI, "Attempt to manipulate access control path", "Attempt to manipulate access control path: " & arguments.path));
			}
			...
			if(structKeyExists(local, "rule") && overlap(local.rule.roles, arguments.roles)) {
				return local.rule;
			}
	...
	<cffunction access="private" returntype="any" name="searchForClassRule" output="false"
	            hint="Rule: Search for rule. Searches the specified access map to see if any of the roles specified have access to perform the specified action on the specified Class.">
		<cfargument type="Struct" name="map" required="true" hint="the map containing access rules"/>
		...
		<cfscript>
			...
			if((structKeyExists(local, "rule")) && (overlapClass(local.rule.actions, arguments.action)) && (overlap(local.rule.roles, arguments.roles))) {
				return local.rule;
			}
			return;
		</cfscript>
	...
	<cffunction access="private" returntype="boolean" name="overlap" output="false"
	...
	<cffunction access="private" returntype="boolean" name="overlapClass" output="false"
			...
			try {
				...
				while(structKeyExists(local, "line")) {
					if(local.line.length() > 0 && local.line.charAt(0) != chr(35)) {
						local.rule = newComponent("cfesapi.org.owasp.esapi.reference.accesscontrol.Rule").init();
